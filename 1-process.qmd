...
```{r}
data_raw_sex_gsk_010 <- readxl::read_xlsx(
  projr::projr_path_get(
    "data-raw-private", "day-2024_07_11-GSK Participants - Blood results sonder names-sex_only.xlsx"
  )
)
data_tidy_sex_gsk_010 <- data_raw_sex_gsk_010 |>
  janitor::clean_names() |>
  dplyr::mutate(
    across(where(is.character), tolower)
  )
UtilsDataRSV::view_cols(data_tidy_sex_gsk_010)
```
...
```{r}
gsk_tbl_010 <- main_tbl |>
  dplyr::filter(`Protocol ID` == "GSK.010")
gsk_tbl_010_pid <- gsk_tbl_010 |>
  dplyr::group_by(PatientID) |>
  dplyr::slice(1) |>
  dplyr::select(PatientID) |>
  dplyr::ungroup()
...
```{r}
data_tidy_sex_gsk_010 
```

So, it seems that we can match the pids by just converting them to to numbers to remove the leadings zeros:

```{r}
gsk_tbl_010_join <- gsk_tbl_010 |>
  dplyr::left_join(
    data_tidy_sex_gsk_010 |>
      dplyr::select(study_no, sex, quantiferon, mantoux, group) |>
      dplyr::rename(PatientID = study_no) |>
      dplyr::mutate(PatientID = as.character(as.numeric(PatientID))),
    by = "PatientID" 
  )
```

We matched TST cutoffs from the sex data to the infection status data perfectly:

```{r}
gsk_tbl_010_join |>
  dplyr::arrange(PatientID) |>
  dplyr::select(
    PatientID, Infxn, quantiferon, mantoux, sex
  ) |>
  dplyr::group_by(PatientID) |>
  dplyr::slice(1) |>
  dplyr::ungroup() |>
  dplyr::filter(
    Infxn == "no" & as.numeric(mantoux)  >= 10 | (
      Infxn == "yes" & as.numeric(mantoux) < 10
    )
  )
```
...
Okay, so we have only four uninfected males, but we do have at least ten in every other group. This looks good.

```{r}
#data_tidy_m72_tb010 <-
data_tidy_m72_tb010_init <- gsk_tbl_010_join |>
  dplyr::mutate(
    prid = "tb010",
    vaccine = "m72",
    ptid = PatientID
  ...
  dplyr::select(
    vaccine, prid, ptid, infxn, sex, timepoint, everything()
  ) |>
  dplyr::select(
    -c(
      PatientID, `Protocol ID`, Vaccine,
      quantiferon, mantoux, group
    )
  )
```

```{r}
data_tidy_m72_tb010_pre_anon <- data_tidy_m72_tb010_init |>
  dplyr::select(-starts_with("Tot")) |>
  tidyr::pivot_longer(
    -c(ptid, prid, vaccine, infxn, sex, timepoint),
    names_to = "cyt_combn",
    values_to = "response"
  ) |>
  dplyr::mutate(
...
set.seed(3)
data_tidy_m72_tb010 <- data_tidy_m72_tb010_pre_anon |>
  dplyr::mutate(
    ptid = .anonymise(ptid, vaccine, prid)
  ) |>
