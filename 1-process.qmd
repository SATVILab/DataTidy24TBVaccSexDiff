...
h56_tbl_non_sex_merge <- h56_tbl_tmp |>
  ...
  dplyr::select(-stim)
ptid_vec_immune <- unique(h56_tbl_non_sex_merge$ptid)
ptid_vec_sex <- unique(
...
ptid_vec_immune
ptid_vec_sex
ptid_vec_main <- unique(h56_tbl_pid$ptid) |> tolower()
ptid_vec_main <- paste0(
  "id_0", unique(h56_tbl_pid$ptid) |> tolower()
)
ptid_vec_main %in% ptid_vec_immune
sum(ptid_vec_immune %in% ptid_vec_sex)
...
summed_response1 <- data_tidy_vacc_freq |>
  group_by(ptid,timepoint,subset) |>
  summarize(total_response = sum(response))
```

```{r}
library(ggplot2)

summed_response1$timepoint <- as.factor(summed_response1$timepoint)
#plot
plot=ggplot(summed_response1, aes(x = ptid, y = total_response, fill = timepoint)) +
...
```{r}
summed_response2 <- data_tidy_vacc_freq |>
  group_by(ptid,timepoint,subset,vaccine,infxn) |>
  summarize(total_response = sum(response))
```

```{r}
summed_response2$timepoint <- as.factor(summed_response2$timepoint)

#plot
plot=ggplot(summed_response2, aes(x = ptid, y = total_response, fill = timepoint)) +
  geom_bar(stat = "identity") +
  facet_wrap(~subset+vaccine+infxn) +
  labs(
    title = "Summed Response",
    x = "Ptid",
    y = "Total Response"
...
```{r}
summed_response3 <- data_tidy_vacc_freq |>
  group_by(ptid,timepoint,subset,vaccine,infxn,sex) |>
  summarize(total_response = sum(response))
```

```{r}
summed_response3$timepoint <- as.factor(summed_response3$timepoint)

#plot
plot=ggplot(summed_response3, aes(x = ptid, y = total_response, fill = timepoint)) +
  geom_bar(stat = "identity") +
  facet_wrap(~subset+vaccine+infxn+sex) +
  labs(
    title = "Summed Response",
    x = "Ptid",
    y = "Total Response"
...
summed_response4 <- data_tidy_vacc_freq |>
  group_by(vaccine,timepoint,ptid,subset) |>
  summarize(total_response = sum(response))
```

```{r}
summed_response4$timepoint <- as.factor(summed_response4$timepoint)

#plot
plot=ggplot(summed_response4, aes(x = vaccine, y = total_response, fill = timepoint)) +
  geom_bar(stat = "identity") +
  facet_wrap(~subset) +
  labs(
    title = "Summed Response",
...
```{r}
summed_response5 <- data_tidy_vacc_freq |>
  group_by(vaccine, timepoint, ptid, subset) |>
  summarize(total_response = sum(response), .groups = "drop")
```

```{r}
summed_response5$timepoint <- as.factor(summed_response5$timepoint)

plot_tbl <- summed_response5 |>
  dplyr::group_by(
    vaccine, timepoint, ptid
  ) |>
  dplyr::mutate(
...
```{r}
summed_response6 <- data_tidy_vacc_freq |>
  group_by(vaccine,timepoint,subset,sex) |>
  summarize(total_response = sum(response))
```

```{r}
summed_response6$timepoint <- as.factor(summed_response6$timepoint)

#plot
plot=ggplot(summed_response6, aes(x = vaccine, y = total_response, fill = timepoint)) +
  geom_bar(stat = "identity") +
  facet_wrap(~sex) +
  labs(
    title = "Summed Response",
