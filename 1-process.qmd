...
# write a loop from 1 to 10 and print out every number squared
for (i in 1:10) {
  print(i^2)
}
```
...
```{r}
data("join17ExcTbl", package = "DataTidy19RodoVaccComp")
...
```{r}
data_tidy_old_summed <- data_tidy_old |>
  dplyr::group_by(vaccine, infxn, ptid, subset, timepoint) |>
  dplyr::summarise(resp = sum(resp), .groups = "drop") 
```

```{r}
data_tidy_new_summed <- data_tidy_vacc_freq |>
  dplyr::inner_join(
    data_tidy_old_comp |>
      dplyr::select(vaccine, infxn) |>
      dplyr::group_by(vaccine, infxn) |>
  ...
  dplyr::summarize(resp = sum(response), .groups = "drop")
```

```{r}
data_tidy_comp_plot <- data_tidy_new_summed |>
  dplyr::mutate(type = "old") |>
  dplyr::bind_rows(
    data_tidy_old_summed |>
...
path_p <- projr::projr_path_get(
  "output-private", "fig", "process", "p-comparison-summed.png"
)
ggsave(
  path_p,
...
- Responses in the new study seem to be consistently lower. Did we subtract unstim more times?