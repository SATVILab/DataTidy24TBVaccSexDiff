...
Here we use the as-is responses, i.e. without subtracting day zero.
...
path_plot_summed_longitudinal <- projr::projr_path_get(
  "output-private", "fig", "exp", "p-summed-raw-longitudinal.png"
)
...
Here we subtracted the day-zero response.
If, for a given participant, there is no day zero response, then we subtract the median day zero response.

```{r}
for (x in list.files("R", full.names = TRUE)) {
  source(x)
}
# debugonce(calc_vaccine_induced_response)
data_tidy_vacc_summed_vi <- data_tidy_vacc_summed |>
  calc_vaccine_induced_response(cn_resp = "total_response")
```

Let's view the data to check it looks okay:

Let's check that all day 0 values are zero:
```{r}
data_tidy_vacc_summed_vi |>
  dplyr::filter(timepoint == 0) |>
  dplyr::pull(total_response) |>
  unique()
```

Let's check that there are no NAs:

```{r}
data_tidy_vacc_summed_vi |>
  dplyr::filter(is.na(total_response))
```

### Longitudinal

```{r}
plot_tbl <- data_tidy_vacc_summed_vi |>
  dplyr::group_by(
    vaccine, timepoint, infxn
  ) |>
  dplyr::summarise(
...
# save as an output
path_plot_summed_longitudinal <- projr::projr_path_get(
  "output-private", "fig", "exp", "p-summed-induced-longitudinal.png"
)
